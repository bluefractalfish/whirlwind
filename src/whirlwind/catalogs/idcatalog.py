"""whirlwind.catalogs.idcatalog 

PURPOSE: wrapper for writing and reading from/to catalog of file ids/uris
...
PUBLIC:
    - IDCatalog(path: str | Path, extension: str = "csv", file_types: Tuple[str,...]=(".tif",".tiff"))
...
@dataclass 
class IDCatalog:
    """ class for writing catalog of file_types with (id, uris )to be used as alternative for walking through directory """
    ...
    @classmethod 
    def write_now(cls, dest: Path, src: Path, file_types=(".tif",".tiff") ) -> "IDCatalog":
        catalog = cls(path=Path(dest), file_types=file_types) 
        catalog.write_from(Path(src))
        return catalog
        ...
        if p.is_file() and p.suffix.lower() == ".csv":
            with p.open("r", newline="", encoding="utf-8") as f:
                ...
                if "id" not in (r.fieldnames or []):
                    raise ValueError(f"IDCatalog missing id column: {p}")
                for row in r:
        ...
        if p.is_file() and p.suffix.lower() == ".csv":
            with p.open("r", newline="", encoding="utf-8") as f:
                ...
                if "uri" not in (r.fieldnames or []):
                    raise ValueError(f"IDCatalog missing uri column: {p}")
                for row in r:
