...
def sample_band(
    ...
    step = max(1, int(math.sqrt(total_tiles / need)))
    sampled = 0
...
def quantize_tile(
    ...
    if qp.scale == "none":
        if qp.dtype.lower() == "float32":
            return arr.astype(np.float, copy=False), {"scale": "none", "dtype": "float32"}
...
def ingest_tiles(
    ...
    toolbox.mkdir(out_dir)
    shards_dir = out_dir/ "shards"
    toolbox.mkdir(shards_dir)
    ...
    with paint.progress("ingesting tiles to", out_dir) as progress:
            ...
            try:
                with rasterio.open(uri) as ds:
                    ...
                    if qp.scale != "none":
                        ...
                        band_bounds = sample_band_stats(ds, tile_size, stride, qp)

                    for r_i, c_i, win in iter_windows(ds, tile_size, stride, drop_partial):
                        tid = tile_id(mosaic_id, r_i, c_i)
                        total_tiles += 1
                        ...
                        try:
                            ...
                            out_arr, q_meta = scale_or_quantize_tile(arr, qp, band_bounds)

                            # Build per-tile georef metadata
                            t_transform = rasterio.windows.transform(win, ds.transform)
                            minx, miny, maxx, maxy = window_bounds(ds, win)
                            ...
                            meta = {
                                ...
                                "crs": crs_str,
                                "transform": affine_to_list(t_transform),
        ...
        toolbox.log(f"ingestion tiles done | written={total_written} skipped={total_skipped} errors={total_errors}")
     return 0
